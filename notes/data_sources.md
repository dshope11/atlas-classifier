...
For an H->WW->2lnunu 0-jet analysis, signal is **H->WW->lvlv** in all production modes (ggF, VBF, WH, ZH, ggZH); background is **fully-leptonic WW (Sh_2212_llvv_os)** plus **dileptonic ttbar (PhPy8 nonallhad)**.

### Signal samples (H->WW->2lnunu, all production modes)
...
|---|---|---|---|---|---|
| WW_llvv | 700602 | Sherpa 2.2.12 | 12.079 | 900,000 | Fully-leptonic WW->lvlv, opposite-sign |
...
For an H->WW->2lnunu analysis (2-lepton channel):
...
- **WW->lvlv** (`Sh_2212_llvv_os`, DSID 700602) is the irreducible diboson background - same final state as the signal, distinguished only by kinematics. This is the equivalent of the old release's `llvv` (DSID 363492).
...
Samples to **avoid**:
- DSIDs 341456/341458/341460 (ZH->VnuWW->lvqq) - semi-leptonic WW final state, wrong for 2-lepton channel
...
|---|---|---|
| `lumi` | external constant | 36.1 fb^-1 * 1000 = 36100 pb^-1 (see below) |
...
## Luminosity value: 36.1 fb^-1

We use `lumi = 36.1 fb^-1` as the default in `data_loading.py`. The picture across sources:

| Source | Value | Notes |
|---|---|---|
| ATLAS-published precise value (2015+2016) | **36.1 fb^-1** | From the ATLAS luminosity publication; what we use |
| Official ATLAS O&E docs | 36 fb^-1 | Rounded for general readers |
| 2025 release `ttbar_analysis.ipynb` | 36000 pb^-1 = 36 fb^-1 | Notebook convention; rounded |
| Older 2020 release `HZZAnalysis.ipynb` | 36.6 fb^-1 | Pre-revision estimate - do not use |
...
Sample selection (which DSIDs constitute "signal" vs. "background" for the 2-lepton 0-jet channel) is our own decision based on the physics of the H->WW->2lnunu channel - there is no template H->WW notebook in the O&E repo to copy from.
